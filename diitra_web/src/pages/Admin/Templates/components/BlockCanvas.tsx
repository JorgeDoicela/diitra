...
const SortableBlockItem: React.FC<SortableBlockItemProps> = ({
    ...
    return (
        <div
            ...
            style={style}
            onClick={() => onSelectBlock(block.id)}
            className={`group relative border rounded-md bg-surface transition-all cursor-pointer ${isCover ? 'p-0' : 'p-4'}
                ${isDragging ? 'shadow-lg border-border-hover opacity-95 scale-[1.002]' : ''}
                ${isActive
                    ? 'border-text-main ring-1 ring-text-main'
                    : isCover
                        ? 'border-transparent'
                        : 'border-border-thin/60 hover:border-border-hover'
                }
            `}
        >
    ...
    return (
        <div className="flex-1 flex flex-col min-h-0 bg-bg-deep border border-border-thin rounded-md overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-8" style={{ scrollbarWidth: 'thin' }}>
                ...
                <div className="max-w-[794px] mx-auto bg-surface p-12 shadow-sm border border-border-thin min-h-[1123px] rounded-sm relative flex flex-col gap-2">
