            ...
            <div className={`transition-all duration-300 ease-in-out origin-top shrink-0 relative ${headerCollapsed
                ...
                {selectedTemplate && !headerCollapsed && (
                    <div className="absolute bottom-1 right-0 flex items-center gap-2.5 z-30 animate-fade-in">
                        {/* Paleta de bloques tipo Notion (Solo se renderiza si no es GLOBAL_THEME) */}
                        ...
                        <button
                            ...
                            disabled={saving}
                            className="flex items-center gap-1.5 px-4 py-1.5 rounded-md bg-text-main text-bg-deep text-xs font-medium hover:opacity-90 transition-all shadow-none disabled:opacity-50 cursor-pointer animate-fade-in"
                        >
                            {saving ? (
                            ...
                            {selectedTemplate.code === 'GLOBAL_THEME' ? 'Guardar Diseño Global' : `Guardar y Publicar v${selectedTemplate.version}`}
                        </button>
                    </div>
                )}
            </div>
